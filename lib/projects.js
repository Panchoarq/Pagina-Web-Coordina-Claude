...
// PROJECTS_FALLBACK: se usa solo si Airtable no responde (o falta el
// token). En uso normal, todo el contenido viene de Airtable.
export const PROJECTS_FALLBACK = RAW.map(withMedia);

// No se cachea aca a mano -- el fetch de lib/airtable.js ya usa
// `next: { revalidate: 60 }`, que es el ISR real de Next.js.
export async function getProjects() {
  try {
    const { getAirtableProjects } = await import("./airtable");
    const live = await getAirtableProjects();
    return live && live.length ? live.map(withMedia) : PROJECTS_FALLBACK;
  } catch (err) {
    console.warn("[coordina] No se pudo cargar Airtable, se usa el respaldo local:", err.message);
